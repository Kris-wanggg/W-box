...
// ─── NewPasswordPage ─────────────────────────────────────────────────────────
// Two views:
//   form    → 設定新密碼 with password + confirm fields   (Dark/newtpassword)
//   success → all-green strength badges, 完成 clicked     (Dark/newtpassword/finish)
...
function PasswordField({ id, label, value, onChange, show, onToggleShow, checks }) {
  return (
      ...
      <div className="flex flex-col gap-2">
        <div className="relative">
          <input
            ...
            value={value}
            onChange={onChange}
            placeholder={id === 'password' ? '請輸入密碼' : '再次輸入密碼'}
    ...
    >
      <div className="w-full max-w-[440px] bg-surface border border-border rounded-xl px-10 py-7 shadow-[0px_8px_32px_0px_rgba(0,0,0,0.12)]">
        ...
        <div className="flex flex-col gap-2 mb-6">
          <h1 className="text-heading-h1 font-semibold text-text-primary text-center w-full">設定新密碼</h1>
          <p className="text-body-base text-text-secondary text-center w-full">請在下方輸入您的新密碼</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-3 mt-2">
            <button
              type="submit"
              disabled={!allPassed}
              className="w-full py-[14.5px] rounded-md bg-primary hover:opacity-90 active:opacity-80 disabled:opacity-40 disabled:cursor-not-allowed text-button-primary font-semibold text-white transition-opacity duration-200 cursor-pointer"
            >
              完成
            </button>
            <a
