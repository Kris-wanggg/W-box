    ...
    <main className="min-h-screen flex items-center justify-center bg-[var(--bg)] transition-colors duration-200 px-4 py-8">
      <div className="w-full max-w-md">
        ...
        <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl shadow-xl p-8 transition-colors duration-200">
          ...
          {/* Tabs */}
          <div className="flex rounded-xl overflow-hidden border border-[var(--border)] mb-6" role="tablist" aria-label={t.registerTitle}>
            <button type="button" onClick={() => setTab('b2c')} aria-selected={tab === 'b2c'} role="tab"
              className={`flex-1 py-2.5 text-sm font-medium transition-colors duration-200 ${
                tab === 'b2c' ? 'bg-orange-500 text-white' : 'text-[var(--muted)] hover:text-[var(--text)] hover:bg-[var(--border)]'
              }`}>
              {t.tabB2C}
            </button>
            <button type="button" onClick={() => setTab('b2b')} aria-selected={tab === 'b2b'} role="tab"
              className={`flex-1 py-2.5 text-sm font-medium transition-colors duration-200 ${
                tab === 'b2b' ? 'bg-orange-500 text-white' : 'text-[var(--muted)] hover:text-[var(--text)] hover:bg-[var(--border)]'
              }`}>
              {t.tabB2B}
            </button>
          </div>
          ...
          <div className="space-y-4">
            {tab === 'b2c' ? (
              <>
                <InputField id="name" label={t.nameLabel} placeholder={t.namePlaceholder} />
                <InputField id="email" label={t.emailLabel} placeholder={t.emailPlaceholder} type="email" />
                <InputField id="phone" label={t.phoneLabel} placeholder={t.phonePlaceholder} type="tel" />
                <PasswordField id="password" label={t.passwordLabel} placeholder={t.passwordPlaceholder}
                  show={showPassword} onToggle={() => setShowPassword(!showPassword)}
                  value={password} onChange={setPassword} showStrength />
                <PasswordField id="confirm-password" label={t.confirmPasswordLabel} placeholder={t.confirmPasswordPlaceholder}
                  show={showConfirmPassword} onToggle={() => setShowConfirmPassword(!showConfirmPassword)} />
              </>
            ) : (
              <>
                <InputField id="company" label={t.companyLabel} placeholder={t.companyPlaceholder} />
                <InputField id="tax-id" label={t.taxIdLabel} placeholder={t.taxIdPlaceholder} />
                <InputField id="contact-name" label={t.contactLabel} placeholder={t.contactPlaceholder} />
                <InputField id="company-email" label={t.corpEmailLabel} placeholder={t.corpEmailPlaceholder} type="email" />
                <InputField id="phone" label={t.phoneLabel} placeholder={t.phonePlaceholder} type="tel" />
                <PasswordField id="password" label={t.passwordLabel} placeholder={t.passwordPlaceholder}
                  show={showPassword} onToggle={() => setShowPassword(!showPassword)}
                  value={password} onChange={setPassword} showStrength />
          ...
          <div className="flex items-center my-5">
            ...
            <span className="mx-4 text-sm text-[var(--muted)]">{t.orContinueWith}</span>
            <div className="flex-1 border-t border-[var(--border)]" />
          </div>

          <div className="space-y-3">
            <button type="button" aria-label={t.registerGoogle}
            ...
            <button type="button" aria-label={t.registerLine}
...
function InputField({ id, label, placeholder, type = 'text' }: {
  ...
  return (
    <div>
      <label htmlFor={id} className="block text-sm font-medium text-[var(--text)] mb-1">{label}</label>
...
function PasswordField({ id, label, placeholder, show, onToggle, value, onChange, showStrength = false }: {
  id: string; label: string; placeholder: string; show: boolean; onToggle: () => void
  value?: string; onChange?: (v: string) => void; showStrength?: boolean
}) {
  return (
    <div>
      <label htmlFor={id} className="block text-sm font-medium text-[var(--text)] mb-1">{label}</label>
      <div className="relative">
        <input id={id} type={show ? 'text' : 'password'} placeholder={placeholder} aria-label={label}
          value={value ?? undefined} onChange={onChange ? (e) => onChange(e.target.value) : undefined}
        ...
        <button type="button" onClick={onToggle} aria-label={show ? '隱藏密碼' : '顯示密碼'}
